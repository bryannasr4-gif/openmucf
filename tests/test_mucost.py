...
def test_tier_spread_is_about_three_orders_of_magnitude_on_mixed_bases(table):
    ...
    as the one assert that states the order of magnitude directly -- the medians themselves are
    pinned exactly below, and ``test_no_aggregate_admits_a_mu_plus_only_row`` pins the T3/T1
    quotient to one decimal -- but it is deliberately loosened to an order-of-magnitude band,
    because a tight bound would once again be pinning a precision the bases do not support.
    """
    m1 = table.tier_median("T1-design-study")
    m3 = table.tier_median("T3-operating-facility")
    spread = m3 / m1
