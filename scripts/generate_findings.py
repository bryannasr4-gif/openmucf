...
from openmucf import uq  # noqa: E402
...
def _tbl2(dc, de, rel):
    ...
    return header + "\n".join(f"| {k} | {dc[k]:.3f} | {de[k]:.3f} |" for k in keys)
...
{_tbl([(k, f"{v:.3f}") for k, v in xmu_rank])}

**Q_net (net-electrical gain):**
{_tbl([(k, f"{v:.3f}") for k, v in q_rank])}
...
|---|---|---|---|
| X_mu | {fw["X_mu"]["lo"]:.0f} | {fw["X_mu"]["med"]:.0f} | {fw["X_mu"]["hi"]:.0f} |
| Q_sci | {fw["Q_sci"]["lo"]:.3f} | {fw["Q_sci"]["med"]:.3f} | {fw["Q_sci"]["hi"]:.3f} |
| Q_net | {fw["Q_net"]["lo"]:.4f} | {fw["Q_net"]["med"]:.4f} | {fw["Q_net"]["hi"]:.4f} |

P(Q_sci > 1) = {fw["P_Qsci_gt1"] * 100:.1f}% ; P(Q_net > 1) = {fw["P_Qnet_gt1"] * 100:.1f}%.
...
- **P(X_mu > 500) = {be["P_xmu_gt500"] * 100:.1f}%**, P(Q_sci > 2) = {be["P_qsci_gt2"] * 100:.1f}%,
...
- Even at **zero sticking**, the best measured cycling rate ($\\lambda_c$=1.45e8) caps the yield at
  **X_mu = {be["xmu_cap_at_measured_lambda_c"]:.0f}** at liquid density; even at the +30% reproduction
