...
> lambda_c band -- so the headline results in sections 1 and 2 depend only on measured rates and the muon lifetime, not on
...
> room-temperature thermal rate, so the section-1c eta bracket, which runs the full network, is not
...
both lie above it, and neither is put there by a density or a temperature this box excludes:
...
density dependence, so the disagreement is about sticking.
