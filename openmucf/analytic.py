...
LAMBDA_0_DEFAULT = 4.552e5  # s^-1, muon decay rate; ledger key 'lambda_mu_decay'
E_F_MEV_DEFAULT = 17.6  # MeV per d-t fusion
E_MU_GEV_DEFAULT = 5.0  # GeV per muon produced (dominant cost; 2-10 GeV range)
...
def fusions_per_muon(omega_s_eff, lambda_c, lambda_0=LAMBDA_0_DEFAULT):
...
def energy_gain(x_mu, eta_conv, E_f_MeV=E_F_MEV_DEFAULT, E_mu_GeV=E_MU_GEV_DEFAULT):
    """Q = X_mu * E_f * eta_conv / E_mu (units reconciled internally)."""
    E_mu_MeV = E_mu_GeV * 1.0e3
    return x_mu * E_f_MeV * eta_conv / E_mu_MeV


def breakeven_xmu(E_f_MeV=E_F_MEV_DEFAULT, E_mu_GeV=E_MU_GEV_DEFAULT, eta_conv=1.0):
