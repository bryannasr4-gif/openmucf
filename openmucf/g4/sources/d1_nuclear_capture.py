...
#: The *second* compiled-in copy of the same two tables, vendored beside the first at the same
#: upstream revision. It exists in this module only so the two copies can be compared against each
#: other without a Geant4 build; nothing D1 ships is generated from it.
HELPER_RELPATH = "third_party/geant4/v11.4.2/G4MuonicAtomHelper.cc"
HELPER_BLOB_ID = "98935195538c67c24ad1229c6064c8da05c9e7e2"
HELPER_SHA256 = "038a13afafdb23a7a34648659a066359fbaec13c73d0ab31954512c3702ff463"
...
#: Shared by both copies: each declares the record struct with the same tokens, so this one pattern
#: anchors the end of the capture function's comment block in either file.
...
# --------------------------------------------------------------------------------------------
# the two compiled-in copies -- same tables, different declaration syntax, different clamp
# --------------------------------------------------------------------------------------------
...
class SourceCopy(NamedTuple):
    ...
    ``zeff``, show up as a difference in the extraction rather than as a difference in the parser.
    """

    name: str
    caprates_decl: re.Pattern[str]
    zeff_decl: re.Pattern[str]
    maxz_decl: re.Pattern[str]
    coefficient_anchors: tuple[tuple[str, str], ...]
    capture_function: re.Pattern[str]
    zeff_function: re.Pattern[str]


#: The copy every shipped D1 byte is generated from. Its patterns are the ones this module has
#: always used; moving them behind a record must not change a single generated byte.
BOUND_DECAY = SourceCopy(
    name="G4MuonMinusBoundDecay.cc",
    caprates_decl=_CAPRATES_DECL,
    zeff_decl=_ZEFF_DECL,
    maxz_decl=_MAXZ_DECL,
    coefficient_anchors=_COEFFICIENT_ANCHORS,
    capture_function=_CAPTURE_FUNCTION,
    zeff_function=_ZEFF_FUNCTION,
)

#: The helper's ``zmin`` anchor is the one coefficient whose *shape* differs, not just its syntax:
#: BoundDecay clamps ``Z`` into ``[1, maxZ]`` with ``std::max(std::min(...), 1)`` and the helper
#: clamps into ``[0, maxZ]`` with an ``if``. Extracting it as a coefficient rather than special-casing
#: it is what makes the difference a value two extractions can be compared on.
HELPER = SourceCopy(
    name="G4MuonicAtomHelper.cc",
    caprates_decl=re.compile(r"constexpr\s+capRate\s+capRates\s*\[\s*\]\s*=\s*\{"),
    zeff_decl=re.compile(r"constexpr\s+G4double\s+zeff\s*\[\s*maxZ\s*\+\s*1\s*\]\s*=\s*\{"),
    maxz_decl=re.compile(r"constexpr\s+size_t\s+maxZ\s*=\s*([0-9]+)\s*;"),
    coefficient_anchors=(
        ("b0a", r"constexpr\s+G4double\s+b0a\s*=\s*(" + _FLOAT + r")\s*;"),
        ("b0b", r"constexpr\s+G4double\s+b0b\s*=\s*(" + _FLOAT + r")\s*;"),
        ("b0c", r"constexpr\s+G4double\s+b0c\s*=\s*(" + _FLOAT + r")\s*;"),
        ("t1", r"constexpr\s+G4double\s+t1\s*=\s*(" + _FLOAT + r")\s*;"),
        ("xmu_coeff", r"xmu\s*=\s*zeff2\s*\*\s*(" + _FLOAT + r")\s*;"),
        ("mix", r"\(\s*1\.0\s*-\s*\(\s*1\.0\s*-\s*xmu\s*\)\s*\*\s*(" + _FLOAT + r")\s*\)"),
        ("zmin", r"if\s*\(\s*Z\s*<\s*0\s*\)\s*\{\s*Z\s*=\s*([0-9]+)\s*;"),
        ("zmax", r"constexpr\s+size_t\s+maxZ\s*=\s*([0-9]+)\s*;"),
    ),
    capture_function=re.compile(r"G4MuonicAtomHelper::GetMuonCaptureRate\s*\("),
    zeff_function=re.compile(r"G4MuonicAtomHelper::GetMuonZeff\s*\("),
)
...
def _parse_capture_records(
    text: str, masked: str, copy: SourceCopy
) -> tuple[
    ...
    """
    start, stop = _array_body(masked, copy.caprates_decl, "capRates[]")
...
def _parse_zeff(
    ...
    start, stop = _array_body(masked, copy.zeff_decl, "zeff[]")
    values: list[float] = []
    literals: list[str] = []
    lines: list[int] = []
    offset = start
...
def extract(source_text: str, copy: SourceCopy = BOUND_DECAY) -> D1Extraction:
    """Parse one compiled-in copy into everything D1 is generated from.

    The default is :data:`BOUND_DECAY`, the copy every shipped byte comes from; passing
    :data:`HELPER` reads the second copy for comparison only.
    """
    masked = _mask(source_text)

    records, literals, lines = _parse_capture_records(source_text, masked, copy)
    zeff, zeff_literals, zeff_lines = _parse_zeff(source_text, masked, copy)

    max_z_match = _find(copy.maxz_decl, masked, "maxZ")
    max_z = int(max_z_match.group(1))

    coefficients: list[tuple[str, str]] = []
    for name, pattern in copy.coefficient_anchors:
        match = _find(re.compile(pattern), masked, f"the {name!r} fallback constant")
        coefficients.append((name, match.group(1)))

    capture_function = _find(copy.capture_function, masked, "GetMuonCaptureRate")
    capture_struct = _find(_STRUCT_CAPRATE, masked, "struct capRate")
    zeff_function = _find(copy.zeff_function, masked, "GetMuonZeff")
...
def check_helper_pins(data: bytes) -> None:
    """Fail loudly if the vendored helper bytes are not the pinned upstream blob.

    Same discipline as :func:`check_pins`, over the second copy: the consistency comparison is only
    evidence if both sides are known to be upstream's bytes rather than a copy someone edited to
    make the comparison pass.
    """
    if b"\r" in data:
        raise SourceExtractionError(
            f"{HELPER_RELPATH} carries CR bytes: the checkout rewrote its line endings and both "
            "pins below are now meaningless. Check `.gitattributes` for `third_party/geant4/** -text`."
        )
    blob = hashlib.sha1(b"blob %d\0" % len(data) + data, usedforsecurity=False).hexdigest()
    if blob != HELPER_BLOB_ID:
        raise SourceExtractionError(
            f"{HELPER_RELPATH} has git blob id {blob}, not the pinned {HELPER_BLOB_ID}. "
            "A deliberate re-pin belongs in a NEW third_party/geant4/<tag>/ directory -- never as "
            "an overwrite, which destroys the evidence that the previously published dataset was "
            "faithful to the version it claimed."
        )
    digest = hashlib.sha256(data).hexdigest()
    if digest != HELPER_SHA256:  # pragma: no cover -- unreachable while the blob id matches
        raise SourceExtractionError(f"{HELPER_RELPATH} has sha256 {digest}, not {HELPER_SHA256}")
...
def load_helper(path: str | Path) -> D1Extraction:
    """Read the vendored helper, check its pins, and extract its copy of the same tables."""
    data = Path(path).read_bytes()  # binary: the pins are over these exact bytes
    check_helper_pins(data)
    return extract(data.decode("ascii"), HELPER)
